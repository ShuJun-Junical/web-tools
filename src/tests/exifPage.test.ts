...
describe('EXIF 工具页面', () => {
  ...
  it('常用信息以中文名展示并可删除撤销，编辑在全部元数据表格内进行', async () => {
    ...
    class FakeWorker {
      ...
      postMessage(message: {
        id: number;
        action: { type: string; file: File; tag?: string; value?: string };
      }) {
        dialog.workerCalls++;
          ...
          result = {
            ...
            fields:
              action.value === undefined
                ...
                : fields.map((field) =>
                    field.key === action.tag ? { ...field, value: action.value! } : field
                  ),
          };
        queueMicrotask(() =>
    ...
    const help = wrapper
      .findAll('button')
      .find((button) => button.attributes('aria-label') === '什么是「处理软件」');
    ...
    await wrapper
      .findAll('button')
      .find((button) => button.text().startsWith('PNG（'))!
      .trigger('click');
    await wrapper
      .findAll('button')
      .find((button) => button.text().startsWith('ExifIFD（'))!
      .trigger('click');
    await flushPromises();
    // 详细信息行同样提供中文名与圈问号说明
    ...
    await wrapper
      .findAll('button')
      .find((button) => button.attributes('aria-label') === '什么是「白平衡」')!
      .trigger('click');
    await flushPromises();
        ...
        const result: ExifResult =
          action.type === 'inspect'
            ? {
                ...
                file: action.file,
                fields: [
                  { key: 'GPS:GPSLatitude', group: 'GPS', name: 'GPSLatitude', value: '30' },
                ],
              }
