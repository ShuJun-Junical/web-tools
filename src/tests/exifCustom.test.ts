...
describe('ExifTool 自定义字段配置', () => {
  it('为 EXIF 数字标识和 XMP 命名空间生成可写定义', () => {
    ...
    const xmp = customTagConfig({ group: 'XMP-demo', identifier: 'Note', dataType: 'string', namespaceUri: 'https://example.com/demo/' });
    expect(xmp.tag).toBe('XMP-demo:Note');
    expect(xmp.config).toContain('https://example.com/demo/');
  });

  it('空层级和代码注入样式的标识被拒绝', () => {
    expect(() => customTagConfig({ group: '', identifier: 'A', dataType: 'string' })).toThrow('层级');
    expect(() => customTagConfig({ group: 'XMP-demo', identifier: "A';system('x')", dataType: 'string' })).toThrow('字段标识');
  });

  it('拒绝含反斜杠或引号的命名空间 URI', () => {
    expect(() => customTagConfig({ group: 'XMP-demo', identifier: 'Note', dataType: 'string', namespaceUri: "https://example.com/a\\" })).toThrow('命名空间');
    expect(() => customTagConfig({ group: 'XMP-demo', identifier: 'Note', dataType: 'string', namespaceUri: "https://example.com/a'b" })).toThrow('命名空间');
    expect(() => customTagConfig({ group: 'XMP-demo', identifier: 'Note', dataType: 'string', namespaceUri: 'https://example.com/a_b' })).not.toThrow();
  });
