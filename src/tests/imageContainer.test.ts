...
describe('图片容器清理', () => {
  it('PNG 与 APNG 保留图像及动画块，普通清理保留色彩和未知块', () => {
    ...
    expect(inspectImage(strong).blocks.map((block) => block.kind)).toEqual([
      'IHDR',
      'acTL',
      'fcTL',
      'IDAT',
  ...
  it('动画 WebP 清理 EXIF/ICC/未知块，并更新 VP8X 标志', () => {
    const source = webp(
      webpChunk('VP8X', [0x2e, 0, 0, 0, 0, 0, 0, 0, 0, 0]),
      webpChunk('ICCP', [1]),
      webpChunk('ANIM', [0, 0, 0, 0, 0, 0]),
      webpChunk('ANMF', [1, 2, 3]),
      webpChunk('EXIF', [4]),
      webpChunk('ZZZZ', [5])
    );
    const normal = clearImageMetadata(source, 'normal');
  ...
  it('JPEG 保留压缩数据，普通清理保留 ICC 和方向，强力清理移除 APP 区段', () => {
    const source = join(
      Uint8Array.of(0xff, 0xd8, 0xff, 0xe1, 0, 4, 1, 2),
      Uint8Array.of(0xff, 0xe2, 0, 14),
    ...
    expect(
      inspectImage(normal)
        .blocks.filter((block) => !block.image)
        .map((block) => block.kind)
    ).toEqual(['APP1', 'APP2', 'APP14', 'APP5']);
  ...
  it('原始块可变长编辑和新增，不改变图像块；非法十六进制输入报错', () => {
    ...
    expect(
      inspectImage(edited).blocks.find((item) => item.kind === 'tEXt')?.dataEnd
    ).toBeGreaterThan(block.dataEnd);
...
describe('JPEG 方向段注入位置', () => {
  it('普通清理将方向 APP1 注入到 JFIF APP0 之后，无 APP0 时紧随 SOI', () => {
    const source = join(
      ...
      Uint8Array.of(0xff, 0xe0, 0, 16),
      encoder.encode('JFIF\0'),
      Uint8Array.of(1, 2, 1, 0, 0x48, 0, 0x48, 0, 0),
    ...
    expect(
      inspectImage(normal)
        ...
        .slice(0, 3)
    ).toEqual(['SOI', 'APP0', 'APP1']);
    expect(sameImageData(source, normal)).toBe(true);
    const noJfif = clearImageMetadata(
      join(Uint8Array.of(0xff, 0xd8, 0xff, 0xda, 0, 2, 17, 34, 0xff, 0, 51, 0xff, 0xd9)),
      'normal',
      6
    );
    expect(
