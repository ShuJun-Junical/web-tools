...
function countRuns(
  characters: string[],
  isMember: (character: string) => boolean,
): number {
  let count = 0
  let inRun = false
...
export function analyzeText(value: string): TextStatistics {
  ...
  const nonEmpty = normalizedLines.map((line) => Array.from(line).some(
    (character) => !WHITESPACE_RE.test(character),
  ))
  let paragraphs = 0
  ...
  return {
    ...
    mixedWords: hanCharacters + countRuns(
      characters,
