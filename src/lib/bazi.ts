...
function findRelations(pillars: BaziPillar[]) {
  ...
  for (let i = 0; i < pillars.length; i++) {
    for (let j = i + 1; j < pillars.length; j++) {
      ...
      if (ganHe.has(ganKey))
        result.push(
          `${positions[i]}干${pillars[i].gan}与${positions[j]}干${pillars[j].gan}：天干五合`
        );
      if (ganChong.has(ganKey))
        result.push(
          `${positions[i]}干${pillars[i].gan}与${positions[j]}干${pillars[j].gan}：天干相冲`
        );
      const zhiKey = pairKey(pillars[i].zhi, pillars[j].zhi);
      for (const [name, pairs] of zhiRelations) {
        if (pairs.has(zhiKey))
          result.push(
            `${positions[i]}支${pillars[i].zhi}与${positions[j]}支${pillars[j].zhi}：${name}`
          );
      }
      if (ziMaoXing.has(zhiKey))
        result.push(`${positions[i]}支${pillars[i].zhi}与${positions[j]}支${pillars[j].zhi}：相刑`);
  ...
  const addGroup = (group: string, name: string) => {
    ...
    const members = values
      .map((value) => `${positions[branches.indexOf(value)]}支${value}`)
      .join('、');
  ...
  sanXing.forEach((group) => {
    ...
    for (let i = 0; i < values.length; i++) {
      for (let j = i + 1; j < values.length; j++) {
        ...
        if (left >= 0 && right >= 0)
          result.push(`${positions[left]}支${values[i]}与${positions[right]}支${values[j]}：相刑`);
      }
    }
  });
  return result;
...
export function calculateBazi(input: BaziInput): BaziResult {
  ...
  const pillars = rawPillars.map(
    ([label, gan, zhi, ganShiShen, hidden, hiddenShiShen, naYin, diShi, xun, xunKong]) => ({
      label,
      gan,
      zhi,
      ...
      ganShiShen,
      hiddenGan: hidden.map((value, index) => ({
        gan: value,
        element: ganElement[value],
        shiShen: hiddenShiShen[index],
      })),
      naYin,
      diShi,
      xun,
  ...
  const fiveElements = {
    gan: emptyElementCounts(),
    zhi: emptyElementCounts(),
    hidden: emptyElementCounts(),
  };
