...
const contingencyEquations: ReadonlyArray<
  readonly [ContingencyKey, ContingencyKey, ContingencyKey]
> = [
  ['a', 'b', 'ab'],
  ['c', 'd', 'cd'],
  ['a', 'c', 'ac'],
...
export function solveContingencyTable(input: Partial<Record<ContingencyKey, number>>) {
  const values = Object.fromEntries(
    contingencyKeys.map((key) => [key, input[key] ?? null])
  ) as Record<ContingencyKey, number | null>;
  ...
  do {
    ...
    contingencyEquations.forEach(([leftKey, rightKey, totalKey]) => {
      ...
      if (!missing.length) {
        if (left! + right! !== total)
          [leftKey, rightKey, totalKey].forEach((key) => conflictKeys.add(key));
        return;
      }
      ...
      const key = missing[0];
      const value =
        key === totalKey ? left! + right! : total! - values[key === leftKey ? rightKey : leftKey]!;
      if (value < 0) {
