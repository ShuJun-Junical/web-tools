...
export function interpretYao(positiveCounts: PositiveCount[]) {
  ...
  const changedLines = positiveCounts.map((count, index) =>
    count === 0 ? true : count === 3 ? false : originalLines[index]
  );
  return {
    ...
    changingLines: positiveCounts.flatMap((count, index) =>
      count === 0 || count === 3 ? [index + 1] : []
    ),
  };
}
...
export function castCoins(actionSample = 0): {
  tosses: boolean[][];
