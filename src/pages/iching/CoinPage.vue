...
const originalText = computed(() =>
  reading.value ? hexagramTexts[reading.value.original.number - 1] : null
);
const changedText = computed(() =>
  reading.value ? hexagramTexts[reading.value.changed.number - 1] : null
);
      ...
      <CardHeader class="grid-cols-[minmax(0,1fr)_auto] grid-rows-1 items-center">
        <div>
          ...
          <p class="mt-1 text-sm text-muted-foreground">
            {{
              hasResult ? '已录入，点击铜钱可切换正反。' : '未录入，点击任意铜钱后开始显示结果。'
            }}
          </p>
        </div>
        <div v-if="hasResult" class="flex gap-2">
          <Button
            ...
            @click="copyText(resultText, '已复制完整起卦结果。')"
            >复制结果</Button
          >
        ...
        <p v-if="result && reading" class="mt-4 text-sm" role="status" aria-live="polite">
          {{
            reading.changingLines.length
              ? `动爻：${reading.changingLines.map((line) => lineLabels[line - 1]).join('、')}`
              : '无动爻，变卦与本卦相同。'
          }}
        </p>
        <p v-if="clipboardError" class="mt-3 text-sm text-destructive" role="status">
          {{ clipboardError }}
        </p>
      </CardContent>
    </Card>
    ...
    <template v-if="result && reading">
      <div class="grid gap-4 md:grid-cols-3" aria-live="polite">
        <Card v-for="hexagram in hexagrams" :key="hexagram.label">
          <CardHeader>
            <CardTitle
              >{{ hexagram.label }}·第 {{ hexagram.value.number }} 卦
              {{ hexagram.value.name }}</CardTitle
            >
            <p class="text-sm text-muted-foreground">
              {{ hexagram.value.upperTrigram.image }}{{ hexagram.value.upperTrigram.name }}上·{{
                hexagram.value.lowerTrigram.image
              }}{{ hexagram.value.lowerTrigram.name }}下
            </p>
          </CardHeader>
          <CardContent class="flex justify-center">
            <div class="flex w-32 flex-col-reverse" :aria-label="`${hexagram.value.name}卦象`">
              <div
                v-for="(yang, index) in hexagram.value.lines"
                :key="index"
                class="relative flex h-3.5 items-center justify-center"
              >
                ...
                <span
                  v-if="
                    hexagram.showChanges &&
                    (result.positiveCounts[index] === 0 || result.positiveCounts[index] === 3)
                  "
                  class="absolute right-0 text-center text-sm font-bold leading-none text-primary"
                  >{{ result.positiveCounts[index] === 0 ? '×' : '○' }}</span
                >
              </div>
            </div>
          </CardContent>
        ...
        <CardContent class="space-y-6">
          ...
          <section v-if="reading.changingLines.length">
            ...
            <ul class="mt-3 space-y-2">
              <li
                v-for="line in reading.changingLines"
                :key="line"
                class="rounded-lg bg-muted/50 px-4 py-3 leading-7"
              >
                <p>{{ originalText.lines[line - 1] }}</p>
                <p class="mt-1 text-sm text-muted-foreground">
                  象曰：{{ originalText.xiaoxiang[line - 1] }}
                </p>
              </li>
              <li
                v-if="reading.changingLines.length === 6 && originalText.lines[6]"
                class="rounded-lg bg-muted/50 px-4 py-3 leading-7"
              >
                <p>{{ originalText.lines[6] }}</p>
                <p v-if="originalText.xiaoxiang[6]" class="mt-1 text-sm text-muted-foreground">
                  象曰：{{ originalText.xiaoxiang[6] }}
                </p>
              </li>
            </ul>
          </section>
