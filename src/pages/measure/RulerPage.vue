...
const customCalculatedPpi = computed(() =>
  calculatePpi(Number(displayWidth.value), Number(displayHeight.value), Number(diagonal.value))
);
        ...
        <div v-if="source === 'calibration'" class="flex flex-col gap-4">
          ...
          <div class="overflow-x-auto rounded-lg border bg-muted/40 p-4">
            <div
              ...
              :style="{ width: `${Number(calibrationWidth) || 0}px` }"
              aria-label="银行卡宽度参照条"
            >
              85.60 mm
            </div>
