...
const result = computed(() =>
  hasErrors.value || !sameLength.value ? null : linearCorrelation(x.value.values, y.value.values)
);
...
const relation = computed(() => {
  ...
  const strength =
    absolute >= 0.8 ? '高度' : absolute >= 0.5 ? '中度' : absolute >= 0.3 ? '低度' : '极弱';
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <label class="space-y-2">
            ...
            <Textarea
              ...
              monospace
              placeholder="1&#10;2&#10;3"
              :aria-invalid="x.invalidLines.length > 0"
            />
          </label>
          <label class="space-y-2">
            ...
            <Textarea
              ...
              monospace
              placeholder="2&#10;4&#10;5"
              :aria-invalid="y.invalidLines.length > 0"
            />
          </label>
        </div>
        <p v-if="hasErrors" role="alert" class="text-sm text-destructive">
          <template v-if="x.invalidLines.length"
            >x 的第 {{ x.invalidLines.join('、') }} 行不是有效数字。</template
          >
          <template v-if="y.invalidLines.length">
            y 的第 {{ y.invalidLines.join('、') }} 行不是有效数字。</template
          >
        </p>
        <p v-else-if="!sameLength" role="alert" class="text-sm text-destructive">
          x 和 y 的数据数量必须相同。
        </p>
        <p v-else class="text-sm text-muted-foreground">
          空行会被忽略，至少需要两对数据，且 x、y 都必须有变化。
        </p>
    ...
    <Card aria-live="polite">
      ...
      <CardContent class="space-y-5">
        <p v-if="!result" class="text-sm text-muted-foreground">
          {{
            x.values.length >= 2 && sameLength && !hasErrors
              ? 'x 或 y 没有变化，无法计算相关系数。'
              : '输入有效的成对数据后显示结果。'
          }}
        </p>
          ...
          <dl class="grid gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-lg bg-muted/60 p-3">
              ...
              <dd class="mt-1 font-semibold tabular-nums">
                y = {{ format(result.slope) }}x {{ result.intercept < 0 ? '−' : '+' }}
                {{ format(Math.abs(result.intercept)) }}
              </dd>
            </div>
          </dl>
