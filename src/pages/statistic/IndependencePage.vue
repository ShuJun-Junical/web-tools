...
function emptyCells(): Record<ContingencyKey, string> {
  return Object.fromEntries(contingencyKeys.map(key => [key, ''])) as Record<ContingencyKey, string>
}

const cells = ref(emptyCells())
const inputVersions = ref(Object.fromEntries(contingencyKeys.map(key => [key, 0])) as Record<ContingencyKey, number>)
const hasInput = computed(() => contingencyKeys.some(key => cells.value[key] !== ''))
const invalidKeys = computed(() => contingencyKeys.filter(key => {
  const raw = cells.value[key]
  const value = Number(raw)
  return raw !== '' && (!raw.trim() || !Number.isSafeInteger(value) || value < 0)
}))
const inputValues = computed(() => Object.fromEntries(contingencyKeys.flatMap(key => {
  const raw = cells.value[key]
  return raw === '' ? [] : [[key, Number(raw)]]
})) as Partial<Record<ContingencyKey, number>>)
const solution = computed(() => solveContingencyTable(invalidKeys.value.length ? {} : inputValues.value))
const unresolvedKeys = computed(() => hasInput.value && !invalidKeys.value.length && !solution.value.conflictKeys.length
...
const result = computed(() => invalidKeys.value.length || solution.value.conflictKeys.length || !counts.value
  ? null
  : chiSquare2x2(counts.value))
const association = computed(() => result.value === null || !counts.value ? null : phiCoefficient2x2(counts.value))
const hasZeroTotal = computed(() => Boolean(
  counts.value
...
const formula = computed(() => {
  ...
  if (result.value === null || !counts.value) return katex.renderToString(base, { displayMode: true })
            ...
            <template v-for="(row, rowIndex) in tableRows" :key="rowIndex">
              <label v-for="key in row" :key="key" :for="`cell-${key}`" class="space-y-1">
                ...
                <Input
                  ...
                  :class="[
                    cells[key] === '' && solution.derivedKeys.includes(key) && 'bg-muted/60 text-muted-foreground',
                    hasZeroTotal && totalKeys.includes(key) && solution.values[key] === 0 && 'bg-destructive/10',
                  ]"
