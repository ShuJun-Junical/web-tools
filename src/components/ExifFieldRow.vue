...
const draft = ref('');
watch(() => props.editing, editing => { if (editing) draft.value = props.field.value; });
  ...
  <div class="border-b py-1.5 last:border-b-0">
    <div class="grid items-center gap-x-3 gap-y-1 sm:grid-cols-[13rem_minmax(0,1fr)_auto]">
      <div class="min-w-0">
        ...
        <p v-if="info" class="truncate font-mono text-xs text-muted-foreground/70">{{ field.name }}</p>
      </div>
      <p class="truncate text-sm text-muted-foreground" :title="field.value">{{ field.value }}</p>
      <div class="flex justify-end gap-1">
        <Button variant="ghost" size="sm" :disabled="busy || locked" @click="emit('edit')">编辑</Button>
    ...
    <div v-if="editing" class="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
      ...
      <p v-if="composite" class="text-xs text-muted-foreground sm:col-span-2">该字段是数组或复合值，将以文本写入，类型可能改变。</p>
    </div>
  </div>
</template>
