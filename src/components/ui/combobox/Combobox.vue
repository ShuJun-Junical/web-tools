...
<template>
  <ComboboxRoot v-model="model" open-on-click open-on-focus>
    <ComboboxAnchor
      class="border-input bg-background flex h-9 w-full items-center rounded-md border shadow-xs focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50"
    >
      <ComboboxInput
        v-model="search"
        :display-value="(value) => items.find((item) => item.value === value)?.label ?? ''"
        :aria-label="label"
        :placeholder="placeholder"
        class="h-full min-w-0 flex-1 bg-transparent px-3 text-sm outline-none placeholder:text-muted-foreground"
        inputmode="numeric"
        @blur="commitInput"
        @keydown.enter.prevent.stop="commitInput"
      />
      <ComboboxTrigger class="flex h-full items-center px-2" :aria-label="`展开${label}`"
      ...
      >
        <ComboboxViewport class="p-1">
          ...
          <ComboboxItem
            ...
            :key="item.value"
            :value="item.value"
            class="relative flex cursor-default select-none items-center rounded-sm py-1.5 pr-8 pl-2 text-sm outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground"
          >
