...
function commitInput() {
  ...
  if (
    (props.minValue && date.compare(props.minValue) < 0) ||
    (props.maxValue && date.compare(props.maxValue) > 0)
  ) {
    model.value = null;
...
<template>
  <div>
      ...
      <PopoverRoot v-model:open="open">
        <PopoverTrigger as-child>
          <Button
            ...
            :aria-label="
              model
                ? `打开${label}日历，当前为${formatter.format(model.toDate(getLocalTimeZone()))}`
                : `打开${label}日历`
            "
            @click="openedFromInput = false"
          >
            <CalendarDays />
          </Button>
        </PopoverTrigger>
        <PopoverPortal>
          ...
          >
            <CalendarRoot
              ...
              :week-starts-on="1"
              fixed-weeks
              :initial-focus="!openedFromInput"
              @update:model-value="selectDate"
            >
              <template #default="{ grid, weekDays }">
                <CalendarHeader class="relative flex items-center justify-center pb-3">
                  <CalendarPrev as-child
                    ><Button type="button" variant="ghost" size="icon-sm" class="absolute left-0"
                      ><ChevronLeft /><span class="sr-only">上个月</span></Button
                    ></CalendarPrev
                  >
                  <CalendarHeading class="text-sm font-medium" />
                  <CalendarNext as-child
                    ><Button type="button" variant="ghost" size="icon-sm" class="absolute right-0"
                      ><ChevronRight /><span class="sr-only">下个月</span></Button
                    ></CalendarNext
                  >
                </CalendarHeader>
                <CalendarGrid
                  ...
                  <CalendarGridHead>
                    <CalendarGridRow class="flex">
                      <CalendarHeadCell
                        ...
                        :key="weekDay"
                        :class="showLunar ? 'size-11' : 'size-9'"
                        class="flex items-center justify-center text-xs font-normal text-muted-foreground"
                    ...
                    >
                      ...
                      >
                        <CalendarCellTrigger
                          ...
                          :month="month.value"
                          :class="showLunar ? 'size-11' : 'size-9'"
                          class="flex flex-col items-center justify-center rounded-md text-sm leading-none outline-none hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring data-[selected]:bg-primary data-[selected]:text-primary-foreground data-[today]:font-semibold data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[outside-view]:text-muted-foreground data-[outside-view]:opacity-50"
                        >
                          <span>{{ date.day }}</span>
                          <span v-if="showLunar" class="mt-1 text-[10px] font-normal opacity-70">{{
                            lunarLabel(date)
                          }}</span>
                        </CalendarCellTrigger>
                      </CalendarCell>
    ...
    <div class="min-h-5 pt-1">
      <p v-if="error" role="alert" class="text-xs font-normal text-destructive">{{ error }}</p>
    </div>
  </div>
</template>
